...
import { StudantQuery } from 'src/app/interfaces/studant-query/studant.query';
...
export class StudantService {
     ...
     */
    public async get(query: StudantQuery): Promise<StudantModel> {
        try {
            let response = await this.http.get<Response<StudantModel>>(Routes.STUDANT_GET_ROUTE, { params: { 'userId': query.userId, 'id': query.id } }).toPromise()
    ...
    public async setStudantData(userId: string): Promise<boolean> {
        ...
        try {
            let studant = await this.get(new StudantQuery(null, userId));
